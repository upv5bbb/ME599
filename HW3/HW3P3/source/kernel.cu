...
#define CORRECT_FINAL 1.0437619617094618
...
__device__
float2 RK4(float x, float y, float dt, float tFinal,
float param, float sys) {
	float dx = 0.f, dy = 0.f;
	float k1 = 0.f, k2 = 0.f, k3 = 0.f, k4 = 0.f, y1 = 0.f, y2 = 0.f, y3 = 0.f, y4 = 0.f;
	for (float t = 0; t < tFinal; t += dt){
		float dx = 0.f, dy = 0.f;
		k1 = y;
		y1 = f(x, y, param, sys);
		...
		k2 = y + y1*dt / 2;
		y2 = f(x + k1*dt / 2, y + y1*dt / 2, param, sys);

		k3 = y + y2*dt / 2;
		y3 = f(x + k2*dt / 2, y + y2*dt / 2, param, sys);

		k4 = y + y3*dt;
		y4 = f(x + k3*dt, y + y3*dt, param, sys);
...
void stabImageKernel(uchar4 *d_out, int w, int h, float p, int s) {
	...
	const float2 pos = RK4(x0, y0, TIME_STEP, FINAL_TIME, p, s);
	...
	if (c == w * 55 / 100 && r == h * 55 / 100){
		...
		printf("\nDistance= %f\n", dist_f);
		const float absError = abs(CORRECT_FINAL - dist);
		printf("abs(error) = %f\n", absError);
